...
# Creating a dictionary that maps each word to its count..
...
for word, count in word2count.items():
    if (count >= threshold):
        questionswords2ints[word] = word_number
        answerwords2ints[word] = word_number
        word_number += 1
...
for token in tokens:
    answerwords2ints[token] = len(answerwords2ints) + 1

answersints2word = {w_i: w for w, w_i in answerwords2ints.items()}

# Adding the end of string tokens to the end of every answer
for i in range(len(clean_answers)):
    clean_answers[i] += ' <EOS>'

# Translating all the qustions and answers to numbers
questions_into_ints = []
for question in clean_questions:
    ints = []
    for word in question.split():
...
for answer in clean_answers:
    ints = []
    for word in answer.split():
        if (word not in answerwords2ints):
            ints.append(answerwords2ints['<OUT>'])
        else:
            ints.append(answerwords2ints[word])
...
# Creating the placeholders for the inputs and the targets...
def model_inputs():
...
# Decode the test/validation test
...
def seq2seq_model(inputs, targets, keep_prob, batch_size, sequence_length, answers_num_words, questions_num_words, encoder_embedding_size, decoder_embedding_size, rnn_size, num_layers, questionswords2int):
    ...
    decoder_embedded_inputs = tf.nn.embedding_lookup(decoder_embedding_matrix, preprocessed_targets)
...
# Pad the sequences with the <PAD>
...
def split_into_batches(questions, answers, batch_size):
    for batch_index in range(0, len(questions) // batch_size):
        ...
        padded_answers_in_batch = np.array(apply_padding(answers_in_batch, answerwords2ints))
        yield padded_questions_in_batch, padded_answers_in_batch


# Splitting the questiona and the answers in the training and the validation sets
